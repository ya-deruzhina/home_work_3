...
def create_post(request):
    ...
    post, is_created = Post.objects.get_or_create(id=id,title=title, text=text,counter=counter,category=category,date_create=date_create)
    return JsonResponse (parse_posts(post))
...
#Создаем комментарий с привязкой к посту
@csrf_exempt
def create_comment(request):
    ...
    comment = Comment(text=text, new = post)
    comment.save()
...
def get_comment (request, post_id):
    ...
    result = Post.objects.get(id=post_id).comments.all()
    all_comments = []

    for comment in result:
        all_comments.append ({'id':comment.id, 'text': comment.text, 'new': post_id})

    return JsonResponse ({"Comments": all_comments})

# Меняет статус у всех комментариев по status и id
@csrf_exempt
def change_comment (request):
    post_id = request.POST.get('id')
    comment_status = request.POST.get('status')

    # result = Post.objects.get(id=post_id, status = comment_status).comments.all()
    # all_comments = []

    # for comment in result:
    #     # comment_update = Comment(status = "update")
    #     # comment_update.save()
    #     all_comments.append ({'id':comment.id, 'text': comment.text, 'new': post_id, 'status': comment.status})

    comment = Comment.objects.filter(new=post_id).update(status=comment_status)

    return JsonResponse ({"Comments": "Changed"})
